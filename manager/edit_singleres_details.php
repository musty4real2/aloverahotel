...
$cost=$foodcost * $plate;
	...
	$sql=$object->query("UPDATE `singleresturant` SET `guestid`='$name',`food_id`='$idfood',`plate`='$plate',`cost`='$cost'  WHERE `guestid`='$pid' LIMIT 1");
...
if(isset($_GET['pid'])){
	$targetID=$_GET['pid'];
$sql=mysql_query("SELECT * FROM `singleresturant` WHERE `guestid`='$targetID' LIMIT 1");
...
$auto=$object->query("SELECT `id` , `surname` ,  `othername` FROM `single` WHERE `checkout`='0' ORDER BY
 `surname` DESC");
?>
<form action="edit_singleres_details.php" method="post"  enctype="multipart/form-data" name="myForm" id="myForm">
<center><table width="500" border="0">
<tr>
<td width="142">Name</td>
<td width="234"><?php echo $object->getGuestName($name); ?></td>
<td width="110">
...
echo "<option value='$foodid'> $foodname $foodcategory </option>";
 ...
 ?>
    ...
    <td>&nbsp;&nbsp;<center><?php echo $plate; ?></center></td>
