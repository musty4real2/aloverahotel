...
$co=$n * 500;
	...
	$sql=$object->query("UPDATE `singlegym` SET `guestid`='$id',`num`='$n',`attendant`='$w',`cost`='$co' WHERE `guestid`='$pid' ");
...
if(isset($_GET['pid'])){
	$targetID=$_GET['pid'];
$sql=$object->query("SELECT * FROM `singlegym` WHERE `guestid`='$targetID' LIMIT 1");
...
?>
<form action="singlegym_edit_details.php" method="post"  enctype="multipart/form-data" name="myForm" id="myForm">
...
?>
      <tr class="treven">
<td>Guest name</td>
<td><?php echo $object->getGuestName($name);?> </td>
<td>
