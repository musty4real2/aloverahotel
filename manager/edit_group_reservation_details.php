...
$roomid2=mysql_real_escape_string($_POST['roomid']);
	...
	$sql=$object->query("UPDATE `group` SET  `companyname`='$companyname',`surname`='$surname', `othername`='$othernames', `phonenumber`='$phonenumber', `inidep`='$inideposit', `receptionist`='$receptionist', `occupation`='$occupation', `title`='$title', `arrive_from`='$arrivefrom', `next_destination`='$nextdestination', `checkout_date`='$checkoutdate', `sex`='$sex', `roomid`='$roomid2',`email`='$email',`identification`='$identification',`fulladdress`='$fulladdress' WHERE `id`='$pid' limit 1");

$guestid=mysql_insert_id($connection);
$sql2=$object->query("UPDATE `groupaccountbalance` SET `guestid`='$guestid',`initial_deposit`='$inideposit'  WHERE `guestid`='$pid' limit 1 ");
...
if(isset($_GET['pid'])){
	$targetID=$_GET['pid'];
$sql=mysql_query("SELECT * FROM `group` WHERE `id`='$targetID' LIMIT 1");
...
<form action="edit_group_reservation_details.php" method="post"  enctype="multipart/form-data" name="myForm" id="myForm">

<table width="768" border="0">
    	<tr>
       	  <td width="130"><p class="orange">Basic Info:</p></td>
                         	<td width="137">Company Name</td>
 	<td height="29"><input class="input" type="text" size="35" name="companyname" value="<?php echo $companyname; ?>"/>*</td>
          ...
          <tr>
        	...
        	<td><input type="text" class="input" name="email" size="35" value="<?php echo $email; ?>"/></td>
        	    ...
        	    <td height="70"></td>   
                 ...
                 <td><p class="orange"><center><?php echo $identification; ?></center></p></td>
        	<td width="269">
...
<td>current room</td>
                 <td><p class="orange"><center><?php echo $object->getRoomNumber($roomid2); ?></center></p></td>
