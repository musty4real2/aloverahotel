...
include("../connect.php");
$new_user="";
$sql=mysql_query("SELECT * FROM `user` ORDER BY `entrydate` ASC");
...
if($productCount>0){
	while($row=mysql_fetch_array($sql)){
		...
		$dept=$row['dept'];
		$date_added=strftime("%b %d,%Y",strtotime($row["entrydate"]));
		$new_user.="($id)            $user_name&nbsp;&nbsp; $date_added &nbsp; &nbsp; &nbsp; $dept &nbsp; &nbsp; <a href='user_edit.php?pid=$id'>Edit</a>&nbsp;&bull;<a href='new_user.php?deleteid=$id'>Delete</a><br/>";
	}
	}else{
	$new_user="You have no users listed yet";
}
?>
