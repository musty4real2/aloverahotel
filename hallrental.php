...
$session=mysql_real_escape_string($_POST['session']);
	...
	$year=mysql_escape_string($_POST['year']);
		...
		$sql="INSERT INTO `hallrental` ( `name`, `address`, `phone`, `session`, `amount_paid`, `event_date`, `entry_date`, `entry_time`, `attendant`) VALUES ('$name', '$address', '$phone', '$session', '$money', '$eventdate', NOW(), '$time', '$attendant')";
		...
		header("location:temp_hallrental.php?name=$name&address=$address&phone=$phone&session=$session&money=$money&eventdate=$eventdate&date=$date&time=$time&attendant=$attendant");
...
<td>Name</td>
   ...
   <tr >
    ...
    <tr>
        	<td>session</td>
        <td><input class="input" type="text" size="35" name="session" /></td></tr>
        <tr>
        <tr>
        	<td>Attendant</td>
