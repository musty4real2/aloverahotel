...
$object=new hms();
if($num==0){
	header("location:confirmation.php?cname={$_GET['cname']}");
}
if($num!=0){
	$num=$num-1;
...
//validation
			...
			if(!is_numeric($phonenumber)){$error[]="Amount Collected must be numeric";}
...
$sql=$object->query("INSERT INTO `group` (`companyname`,`surname`, `othername`, `phonenumber`, `inidep`, `receptionist`, `occupation`, `title`, `arrive_from`, `next_destination`, `checkin_date`, `billingno`, `checkout_date`, `sex`, `roomid`,`checkin_time`,`email`,`identification`,`fulladdress`,`room2_id`,`date_of_change`,`checkout`) VALUES ('$cname','$surname', '$othernames', '$phonenumber','$inideposit', '$receptionist', '$occupation', '$title', '$arrivefrom', '$nextdestination', NOW(),'$billing_no', '$checkoutdate', '$sex','$roomid2',NOW(),'$email','$identification','$fulladdress','','','')");

$guestid=mysql_insert_id($connection);
$sql2=$object->query("INSERT INTO `groupaccountbalance` (`guestid`, `entrydate`, `initial_deposit`, `grand_total`, `refund`, `balance`) VALUES ('$guestid', NOW(), '$inideposit', '', '', '')");


header("location:enter_group_reservation_form.php?cname={$_GET['cname']}&num=$num");
}
}
?>
...
<table width="768" border="0">
        ...
        <tr>
          ...
          <tr>
        	...
        	<td><input type="text" class="input" name="nextdestination" size="35" value="<?php if(isset($_POST['nextdestination'])){ echo $_POST['destination'];}?>"/></td>
        </tr>
           	  <tr>
        	    <td height="31"></td>
       	 	 <td>Email Address:</td>
	  ...
	  <tr>
        	    <td height="31"></td>
       	 	 <td>Full Address:</td>
        	<td><input type="text" class="input" name="fulladdress" size="35" <?php if(isset($_POST['fulladdress'])){ echo $_POST['fulladdress'];}?>/></td>
        </tr>
		        <tr>
        	<td></td>
        	<td></td>
...
echo "<option value='$roomid'> $roomnumber $roomcategory $roomlocation</option>";
 ...
 <tr>
        ...
        <tr>
        	...
        	<td><input type="text" class="input" size="35" name="deposit" value="<?php if(isset($_POST['deposite'])){ echo $_POST['deposite'];}?>"/></td>
        </tr>
         <tr>
        	   <td height="29"></td>
       	 	 <td>receptionist:</td>
...
<select name="year" id="year2" class="input">
  ...
  <?php if(isset($_POST['year'])){echo "<option  value='{$_POST['year']}' selected='selected'>{$_POST['year']}</option>"; }?>
	   ...
	   <td></td>
	   <td>  </td>
	   <td height="15"><input type="hidden" value="cname"><input type="hidden" value="num"><input id="button" type="submit" value="Check guest in" name="submit" /></td>
</tr>
 </table>
