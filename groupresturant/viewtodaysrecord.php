...
<title>Veiw Today's group Resturant Record</title>
...
?>  <?php
  ...
  $q = "SELECT * FROM `groupresturant` WHERE `entrydate`= CURDATE() ";
  $r = mysql_query ($q);
...
$fetch="SELECT * FROM `groupresturant` WHERE `entrydate`= CURDATE()  ORDER BY `time` ASC LIMIT $start, $display ";
$fetch=@mysql_query($fetch) or die(mysql_error());
if($object->checkNumofRecord($fetch)==false){echo "<p class='msg info'>Sorry, no visitors found for group resturant today!</p>";}
elseif($object->checkNumofRecord($fetch)==true){
?>

<p>Below are Today's Resturant Visitors</p><br/>
...
<tr>
<td><?php $object->getGuestName2($rows['guestid']);?></td>
