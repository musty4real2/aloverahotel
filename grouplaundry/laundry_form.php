...
$sql=$object->query("INSERT INTO `grouplaundry` (`guestid` ,`cloth_type` ,`amount` ,`cabinetnumber` ,`entrydate` ,`staff` ,`time`,`quantity` ,`return`)
VALUES ('$guestid', '$clothtype', '$amount', '$cabinetnumber', NOW(), '$staff', NOW(),'$quantity','' )");

header("location:temp_grouplaundry_reciept.php?name=$guestid&clothtype=$clothtype&amount=$amount&cabinetnumber=$cabinetnumber&staff=$staff&guesttype=$guesttype&quantity=$quantity");
}
}
?>
...
<td width="44%">Guest name</td>
 <td width="56%"><?php
$auto=$object->query(" SELECT `id` ,`companyname`, `othername`, `surname` FROM `group` WHERE `checkout`='0' ORDER BY 'surname' DESC ");
...
<td><input type="text" class="tablerow" name="quantity" value="<?php if(isset($_POST['quantity'])){ echo $_POST['quantity'];}?>" /></td>
  ...
  <td>&nbsp;</td>
    ...
    <tr>
    <td>&nbsp;</td>
    <td><input type="hidden" value"<?php echo $object->selectGuestType('group', $id); ?>" name="guesttype"/></td>
  </tr>
</table></center>
</form>
<?php 
