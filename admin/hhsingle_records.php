...
<td align="center" width="22%"></td>
 <td width="78%"><a style="color:#000;" href="../logout.php">logout</a>  | <a href="admin_menu.php" style="color:#000;">Home</a></td> 
 </tr>
 </table></center>
<?php
		$id=$_GET['id'];
		...
		$single=$object->query("SELECT * FROM `single` WHERE `id` ='$id' AND`checkout`='1' ");

		
		
        
		$rest=$object->query("SELECT *  FROM `singleresturant` WHERE `guestid`='$id' AND`checkout`='1' ");
	
	
		$gym=$object->query("SELECT *  FROM `singlegym` WHERE `guestid`='$id' AND`checkout`='1' ");

		$laund=$object->query("SELECT * FROM `singlelaundry` WHERE `guestid`='$id' AND`checkout`='1' ");
...
if($ini>$grandtotal){
    ...
    <input type="submit" value="REFUND" onclick="" name="refund"/>
    <?php
	}
if($ini==$grandtotal){
	echo "N0.00";
