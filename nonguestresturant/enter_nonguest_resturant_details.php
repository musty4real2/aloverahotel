...
if($name==""){$error[]="Please Select A Name ";}
	...
	if(!empty($error)){
		...
		elseif(empty($error)){
$a=$object->query("INSERT INTO `outsideresturant` (`guestname`, `food_id`, `plate`, `cost`, `entrydate`, `time`, `waiter`,`checkout`) 
VALUES('$name', '$idfood', '$plate', '$cost',NOW(), NOW(),'$waiter','1')");

 header("location:temp_nonguest_resturant_reciept.php?name=$name&foodid=$idfood&plate=$plate&waiter=$waiter&cost=$cost");
}
}
?>
...
<tr>
<td height="43">Guest name</td>
<td>
<input type="text" name="name" placeholder="E.g Alovera hotels" /></td>
  <tr>              
<td height="47">select Food</td>
 <td><?php
 include('connect.php');
...
echo "<option value='$foodid'> $foodname $foodcategory </option>";
 ...
 ?>
    ...
    <tr>
    <td height="48">Plate quantity:</td>
    <td><select name="plate" class="input">
      ...
      <tr>
    <td height="64">Confirm Plate quantity:</td>
