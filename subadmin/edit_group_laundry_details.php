...
$quantity=mysql_real_escape_string($_POST['quantity']);
	...
	$sql=$object->query("UPDATE `grouplaundry` SET `guestid`='$guestid',`cloth_type`='$clothtype',`amount`='$amount',`cabinetnumber`='$cabinetnumber',`quantity`='$quantity'  WHERE `guestid`='$pid' LIMIT 1");
...
if(isset($_GET['pid'])){
	$targetID=$_GET['pid'];
$sql=mysql_query("SELECT * FROM `grouplaundry` WHERE `guestid`='$targetID' LIMIT 1");
...
<td width="44%" class="align">Guest name</td>
 <td><?php echo  $object->getGuestName2($guestid); ?></td>
 <td width="56%"><?php
 include('../connect.php');
$auto=$object->query("SELECT `id` , `surname`,`othername` FROM `group` WHERE `checkout`='0' ORDER BY 'surname' DESC ");
