...
echo 'Do You Really Want To Delete Guest With ID of '.$_GET['deleteid'].'? <a href="single_resturant_delete.php?yesdelete='.$_GET['deleteid'].'">Yes </a>| <a href="single_resturant_delete.php">No</a>';
exit();
...
if(isset($_GET['yesdelete'])){
	...
	$sql=mysql_query("DELETE FROM `singleresturant` WHERE `guestid`='$id_to_delete' limit 1") or die(mysql_error());
	
	header("location:single_resturant_delete.php");
	exit();
}
...
<?php
		...
		$rest=$object->query("SELECT *  FROM `singleresturant` WHERE`checkout`='0'");
...
<?php 		$t=$object->query("SELECT `roomid`  FROM `single` WHERE `id`='{$row['guestid']}'");
	...
	?>
<td><?php echo "<a href='vsingle_records.php?id={$row['guestid']}&roomid=$roomid'>View Guest Record</a>"; ?> </td> 
<?php echo "<td><a href='single_resturant_delete.php?deleteid={$row['guestid']}'>Delete</a></td>"; ?> 
<?php echo "<td><a href='edit_singleres_details.php?pid={$row['guestid']}'>Edit</a></td>"; ?> 
 </tr>
