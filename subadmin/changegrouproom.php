...
$roomidup=mysql_real_escape_string($_POST['roomid']);
	...
	$sql=$object->query("UPDATE `group` SET  `room2_id`='$roomidup', `date_of_change`=NOW() WHERE `id`='$pid' LIMIT 1");

if($sql){
		$abdul=$object->query("UPDATE `rooms` SET  `room_availability`='1' WHERE `id`='$roomidup' LIMIT 1");
}

header("location:grouproom_occupants.php");
	exit();
}
	?>
    <?php
...
<form action="changegrouproom.php" method="post"  enctype="multipart/form-data" name="myForm" id="myForm">
