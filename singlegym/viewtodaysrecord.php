...
$fetch="SELECT * FROM `singlegym` WHERE `entrydate`= CURDATE()  ORDER BY `time` ASC LIMIT $start, $display ";
$fetch=@mysql_query($fetch) or die(mysql_error());
if($object->checkNumofRecord($fetch)==false){echo "<p class='msg info'>Sorry, no visitors found for gym today!</p>";}
elseif($object->checkNumofRecord($fetch)==true){
?>

<p>Below are Today's Gym Visitors</p>
<center><fieldset class="rfield">
<?php
if(mysql_num_rows($fetch)==0){echo "No Gym Record(s) for today";}
...
echo "<p class='tt'>".N.number_format($resturanttotal, 2)."</p>"; ?></b></center></td>
<td></td>
<td></td>
</tr> 
